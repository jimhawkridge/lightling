...
void chain_follower_init()
{
    ...
    i2c_param_config(I2C_PORT, &conf);
    ESP_ERROR_CHECK(i2c_driver_install(I2C_PORT, conf.mode, I2C_SLAVE_RX_BUF_LEN, I2C_SLAVE_TX_BUF_LEN, 0));
}
...
static void chain_send_fade(uint8_t device, uint8_t chan, uint8_t level, uint16_t time)
{
    ESP_LOGI(TAG, "Request to device %d for fade of %d to %d over %d", device, chan, level, time);
    fade_cmd_t data;
    data.chan = chan;
    data.level = level;
    data.time = time;
...
static int action_from_data(uint8_t *data, int len)
{
    ...
    case I2C_FADE_CMD:
        ...
        ESP_LOGI(TAG, "Fade %d to %d over %d", cmd->chan, cmd->level, cmd->time);
        leds_fade_channel(cmd->chan, cmd->level, cmd->time);
...
{
    int len = i2c_slave_read_buffer(I2C_PORT, rx_buf, I2C_SLAVE_RX_BUF_LEN, 10);
    if (len == -1)
    {
...
void chain_fade_channel(uint8_t channel, uint8_t brightness, uint16_t time)
{
    ...
    ESP_LOGI(TAG, "Request to channel %d on device %d", target_chan, target_addr);
    chain_send_fade(target_addr, target_chan, brightness, time);
}
